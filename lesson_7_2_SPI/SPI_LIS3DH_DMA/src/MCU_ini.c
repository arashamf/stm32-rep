...
//	USART_ITConfig(USART2, USART_IT_RXNE, ENABLE); //включим прерывание на приём при полном регистре данных
	USART2->BRR = (APB_CLOCK + (BAUDRATE/2))/BAUDRATE;
...
uint8_t PLL_ini (uint8_t CPU_WS, uint8_t RCC_PLLM, uint32_t RCC_PLLN, uint32_t RCC_PLLP)
{
	...
	SET_BIT (RCC->CFGR, RCC_CFGR_SW_1); //устанавливается аппаратно для принудительного выбора HSI при выходе из режима остановки/ожидания или в случае выхода из строя генератора HSE. 10: выбран PLL
	SET_BIT (RCC->CFGR, RCC_CFGR_PPRE1_2 | RCC_CFGR_PPRE1_0);// установим делитель шины APB1 - 4 (42 МГц макс. для шины APB1)
	SET_BIT (RCC->CFGR, RCC_CFGR_PPRE2_2 | RCC_CFGR_PPRE2_0);// установим делитель шины APB2 - 4 (84 МГц макс. для APB2)
	RCC->PLLCFGR = RCC_PLLM + (RCC_PLLN << 6) + (RCC_PLLP << 16); 	// HSE/PLLM×PLLN/PLLP. Cдвигаем биты RCC_PLLN и RCC_PLLP для размещения их в нужном месте регистра настройки
	RCC->PLLCFGR |= RCC_PLLCFGR_PLLQ_3; //USB OTG FS clock frequency = VCO frequency / PLLQ, где 2 ≤ PLLQ ≤ 15 (48 МГц макс.)
